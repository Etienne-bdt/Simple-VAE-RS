...
class Cond_SRVAE(BaseVAE):
    def __init__(self, cr, patch_size=64, callbacks=None, slurm_job_id="local"):
        ...
        self.cr = cr
        self.latent_size = int((patch_size * patch_size * 4 / self.cr) // 64) * 64
        ...
        self.encoder_y = nn.Sequential(
            ...
            nn.Conv2d(
                in_channels=128,
                out_channels=self.latent_size_y // 8,
                kernel_size=3,
                stride=1,
                padding=1,
            ),
        ...
        self.decoder_y = nn.Sequential(
            nn.Unflatten(1, (self.latent_size_y // 16, 8, 8)),
            up_block(
                in_channels=self.latent_size_y // 16,
                out_channels=128,
            ),
            up_block(
                in_channels=128,
